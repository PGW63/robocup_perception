#!/home/nvidia/vision_ws/detection_venv/bin/python
"""
...
class GroundedSAM2Node(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('max_depth', 10.0)  # 최대 depth (m)
        ...
        # Segmentation mask publisher (각 객체별 ID로 마스크 생성)
        self.mask_pub = self.create_publisher(Image, '/detection_node/segmentation_mask', 10)
    ...
    def _print_init_info(self):
        ...
        self.get_logger().info(f"      /detection_node/segmentation_mask")
        if self.enable_pointcloud:
    ...
    def _visualize_and_publish(self, img_copy: np.ndarray, header):
        ...
        # 세그멘테이션 마스크 이미지 생성 (각 객체 ID로 라벨링)
        # 0: 배경, 1~N: 각 객체 ID
        ...
        if self.tracking_boxes is not None:
            for i, box in enumerate(self.tracking_boxes):
                ...
                object_id = i + 1  # 객체 ID는 1부터 시작
                
                # Draw bbox
                color = (0, 255, 0)
                cv2.rectangle(img_copy, (x1, y1), (x2, y2), color, 2)
                cv2.putText(img_copy, f"{label} {score:.2f}", (x1, max(0, y1-5)),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
                
                # Draw mask if available and update segmentation mask
                if self.tracking_masks is not None and i < len(self.tracking_masks):
                    mask = self.tracking_masks[i].astype(bool)
                    if mask.shape[:2] == img_copy.shape[:2]:
                        # 시각화용 오버레이
                        overlay_color = np.array([0, 255, 0], dtype=np.uint8)
                        img_copy[mask] = (img_copy[mask] * 0.5 + overlay_color * 0.5).astype(np.uint8)
                        
                        # 세그멘테이션 마스크에 객체 ID 할당
                        segmentation_mask[mask] = object_id
                else:
                    # 마스크가 없으면 bbox 영역을 마스크로 사용
                    segmentation_mask[y1:y2, x1:x2] = object_id
        ...
        if self.enable_pointcloud:
            self._generate_and_publish_pointcloud(segmentation_mask, detections_list, header)
    
    def _generate_and_publish_pointcloud(self, segmentation_mask: np.ndarray, 
        ...
        if self.latest_filtered_depth is None:
            self.get_logger().debug("Waiting for filtered depth image...")
            return
        ...
        try:
            ...
            # 이미지 크기 확인
            if depth_m.shape[:2] != segmentation_mask.shape[:2]:
                self.get_logger().warn(
                    f"Depth shape {depth_m.shape[:2]} != mask shape {segmentation_mask.shape[:2]}")
                return
            ...
            for det in detections:
                ...
                # Detection 인덱스로부터 object_id 추출 (detections 리스트 순서 = object_id)
                det_idx = detections.index(det)
                object_id = det_idx + 1
                
                # 해당 객체의 마스크 추출
                object_mask = (segmentation_mask == object_id)
